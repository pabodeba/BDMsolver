...
class Stokes():
    ...
    def __init__(self, mesh, boundaries):
        ...
        #partial basal sliding 
        x = SpatialCoordinate(self.mesh)
        self.beta = conditional(le(2200.0, x[0]), 0.0, 1e8) + conditional(le(x[0], 2500.0), 0.0, 1e8)    

    def mu(self,u):
        A_rate = Constant(pow(10,-16))   #ice flow rate in SI units
        eps_e = sqrt(0.5*inner(sym(grad(u)), sym(grad(u)))+1e-4)
        return 0.5* pow(A_rate,-1.0/3.0)*pow(eps_e, 1.0/3.0-1.0)   


    def sigma(self, u, p):    #Cauchy stress tensor
        return 2.0*sym(grad(u)) - p*Identity(u.geometric_dimension())

    #bilinear forms for discontinuous weak formulation (interior penalty method)
    def A(self,u, v):
    ...
    def C(self,u,v):
        return -self.beta*inner(u,v)*self.ds(1)    
